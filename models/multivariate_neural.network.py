...
def visualisation(y_pred, Y_test, y_difference):
    ...
    plt.title('Difference in predicted results and actual results')
...
#Neural Network architecture
    def build_regressor():
        ...
        regressor.add(Dense(units=10, kernel_initializer='uniform',activation='relu',input_dim=17 ))
        ...
        regressor.add(Dense(units=2, kernel_initializer='uniform'))
    ...
    max_in_index, max_in_value = max(enumerate(abs_pred[:,0]), key=lambda p: p[1])
    max_del_index, max_del_value = max(enumerate(abs_pred[:,1]), key=lambda x: x[1])
    ...
    print("Largest insertion difference is {0:.2f} at position {1}\nModel predicted {2:.2f} whereas actual result was {3}".format(float(max_in_value), max_in_index, float(y_pred[:,0][max_in_index]), int(Y_test[:,0][max_in_index])))
    print("Largest deletion difference is {0:.2f} at position {1}\nModel predicted {2:.2f} whereas actual result was {3}".format(float(max_del_value), max_del_index, float(y_pred[:,1][max_del_index]), int(Y_test[:,1][max_del_index])))
